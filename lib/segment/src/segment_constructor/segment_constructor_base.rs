...
use crate::types::{
    ...
    SparseVectorStorageType, VectorStorageType,
};
...
fn create_segment(
    ...
    for (vector_name, sparse_vector_config) in &config.sparse_vector_data {
        ...
        let vector_storage = match sparse_vector_config.storage_type {
            // In memory
            SparseVectorStorageType::Memory => {
                let db_column_name = get_vector_name_with_prefix(DB_VECTOR_CF, vector_name);
                open_simple_sparse_vector_storage(database.clone(), &db_column_name)?
            }
            // Mmap on disk, not appendable
            // TODO(sparse) mmap sparse vector storage
