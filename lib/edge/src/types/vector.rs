...
/// let multi: Vector = vec![vec![1.0, 2.0], vec![3.0, 4.0]].into();
///
/// // TryFrom for sparse
...
/// let multi: Vectors = vec![vec![1.0, 2.0], vec![3.0, 4.0]].into();
...
impl Vector {
    ...
    pub fn new_multi(vectors: impl Into<Vec<Vec<f32>>>) -> Self {
        Self(VectorInternal::MultiDense(
            MultiDenseVectorInternal::new_unchecked(vectors.into()),
        ))
    }
}
...
impl From<Vec<Vec<f32>>> for Vectors {
    fn from(vectors: Vec<Vec<f32>>) -> Self {
        Self(VectorStructInternal::MultiDense(
            MultiDenseVectorInternal::new_unchecked(vectors),
        ))
    }
}
